...
void ngx_log_init()
{
    ...
    {
        //没读到，就要给个缺省的路径文件名了
        plogname = "error.log"; //"logs/error.log" ,logs目录需要提前建立出来
    }
    ngx_log.log_level = p_config->GetInt("LogLevel");//缺省日志等级为6【注意】 ，如果读失败，就给缺省日志等级
    if(ngx_log.log_level == -1){
        ngx_log.log_level = 6;
    }
    ...
    ngx_log.fd = open((const char *)plogname.c_str(),O_WRONLY|O_APPEND|O_CREAT,0644);  
