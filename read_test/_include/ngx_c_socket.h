...
public:
	...
	int ngx_epoll_oper_event(int fd,uint32_t eventtype,uint32_t flag,int bcaction,lpngx_connection_t pConn);                                                              //epoll增加事件
...
private:
	...
	void clearMsgSendQueue();                                       //收到一个完整消息后，入消息队列
...
private:
	...
	sem_t                          m_semEventSendQueue;            //收消息队列互斥量 
